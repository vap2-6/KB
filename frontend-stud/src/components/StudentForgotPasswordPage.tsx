...
import { Key, ArrowRight, ArrowLeft, AlertCircle, CheckCircle2, Shield, Info, Mail } from 'lucide-react';
...
export default function StudentForgotPasswordPage({
  initialRegNo = '',
  onBackToLogin
}: StudentForgotPasswordPageProps) {
  ...
  const handleCheckUser = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/student/api/auth/check-user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ register_no: trimmedId })
      });
      ...
      if (res.ok && data.exists) {
        ...
        setError(data.error || 'Student with this Registration Number was not found in database.');
      }
    } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/student/api/auth/forgot-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ register_no: trimmedRegNo, email: trimmedEmail })
      });
    ...
    <div className="min-h-screen w-full flex flex-col bg-[#FFFBF7] font-sans">
      ...
      <header className="bg-white border-b-4 border-[#FA9632] px-6 py-4 flex items-center justify-between shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          <img src={rkmLogo} alt="RKM Crest" className="h-10 w-10 object-contain" />
          <div>
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="max-w-md w-full bg-white border border-[#FCEFD9] rounded-3xl shadow-xl overflow-hidden">
          ...
          <div className="pt-6 pb-1 text-center">
            <h2 className="text-xl font-extrabold tracking-tight text-[#FA9632]">Forgot Password</h2>
          </div>
            ...
            {successMsg ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  />
                  {checkingUser && (
                    <span className="text-[10px] text-amber-600 font-semibold mt-1 block">Checking database for user...</span>
                  )}
                  {userChecked === true && (
                    <span className="text-[10px] text-emerald-600 font-semibold mt-1 flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3" /> Student verified in database
                    </span>
                  )}
                </div>
