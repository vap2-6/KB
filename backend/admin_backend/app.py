    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS invoices (
                ...
                statement_period_start DATETIME NULL,
                statement_period_end DATETIME NULL,
                description TEXT NULL,
                forenoon_count INT DEFAULT 0,
                afternoon_count INT DEFAULT 0,
                total_meals INT DEFAULT 0,
                rate_per_meal DOUBLE DEFAULT 50.0,
                ...
                INDEX idx_inv_student_id (student_id),
...
# --- INVOICES & STATEMENTS ENDPOINTS ---

@admin_bp.route('/invoices', methods=['GET'])
def get_invoices():
    """Fetch invoice records with optional filtering."""
    student_id = request.args.get('student_id')
    status = request.args.get('status')
    ...
    try:
        with conn.cursor() as cur:
            ...
            return jsonify({'success': True, 'invoices': serialize_row(invoices)})
    except Exception as e:
...
def generate_invoice():
    """Generate invoice(s) based on student meal token usage for a specified date range."""
    data = request.json or {}
    student_id = data.get('student_id') # 'all' or specific student_id
    from_date_str = data.get('from_date')
    to_date_str = data.get('to_date')
    invoice_date_str = data.get('invoice_date') or datetime.date.today().isoformat()
    rate_per_meal = float(data.get('rate_per_meal', 50.0))
    tax_rate = float(data.get('tax_rate', 0.0))
    discount_amount = float(data.get('discount_amount', 0.0))
    custom_notes = data.get('notes', 'Meal subscription billing invoice')
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not students:
                return jsonify({'error': 'No eligible student records found'}), 404
            ...
            for st in students:
                sid = st['student_id']
                sname = st['name']
                dept = st.get('grade_section') or st.get('degree_year') or 'General'

                cur.execute("""
                    SELECT 
                        SUM(CASE WHEN meal_type = 'forenoon' THEN 1 ELSE 0 END) as fn_count,
                        SUM(CASE WHEN meal_type = 'afternoon' THEN 1 ELSE 0 END) as an_count,
                        COUNT(*) as total_count
                    FROM meal_tokens 
                    WHERE student_id = %s 
                      AND status IN ('claimed', 'redeemed', 'staff_verified', 'approved')
                      AND created_at >= %s AND created_at <= %s
                """, (sid, from_date_str + " 00:00:00", to_date_str + " 23:59:59"))
                
                usage = cur.fetchone() or {'fn_count': 0, 'an_count': 0, 'total_count': 0}
                fn_c = usage['fn_count'] or 0
                an_c = usage['an_count'] or 0
                tot_c = usage['total_count'] or 0

                if student_id == 'all' and tot_c == 0:
                    continue

                subtotal = float(tot_c * rate_per_meal)
                tax_amt = float(subtotal * (tax_rate / 100.0))
                disc_amt = float(discount_amount)
                total_amt = max(0.0, subtotal + tax_amt - disc_amt)

                inv_code = f"INV-{datetime.datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
                
                cur.execute("""
                    INSERT INTO invoices (
                        invoice_id, student_id, student_name, department, grade_section,
                        invoice_date, statement_period_start, statement_period_end, description,
                        forenoon_count, afternoon_count, total_meals, rate_per_meal,
                        debit_amount, credit_amount, tax_amount, discount_amount, total_amount, balance_due,
                ...
                """, (
                    inv_code, sid, sname, dept, st.get('grade_section'),
                    invoice_date_str + " 10:00:00", from_date_str + " 00:00:00", to_date_str + " 23:59:59",
                    f"Meal Statement Billing ({tot_c} meals: {fn_c} FN, {an_c} AN)",
                    fn_c, an_c, tot_c, rate_per_meal,
                    total_amt, tax_amt, disc_amt, total_amt, total_amt,
                    custom_notes
                ))
                created_invoices.append(inv_code)

            return jsonify({
                'success': True,
                'message': f"Generated {len(created_invoices)} invoice(s) successfully.",
...
@admin_bp.route('/invoices/statement', methods=['POST'])
def generate_bank_statement():
    """Generates bank-statement-style aggregated statement payload."""
    data = request.json or {}
    ...
    try:
        with conn.cursor() as cur:
            ...
            if from_date_str:
                sql += " AND invoice_date >= %s"
                params.append(from_date_str + " 00:00:00")
            if to_date_str:
                sql += " AND invoice_date <= %s"
                params.append(to_date_str + " 23:59:59")
            ...
            customer_info = {
                'name': 'All Students / Combined Ledger' if student_id == 'all' else 'Student Account',
                'id': 'ALL-ACCTS' if student_id == 'all' else student_id,
                'department': 'RKMVC Campus',
                'email': 'N/A'
            }
            ...
            if student_id and student_id != 'all':
                ...
                if st:
                    customer_info = {
                        'name': st['name'],
                        'id': st['student_id'],
                        'department': st.get('grade_section') or 'General',
            ...
            for inv in invoices:
                debit = float(inv.get('debit_amount') or inv.get('total_amount') or 0.0)
                credit = float(inv.get('credit_amount') or (debit if inv.get('status') == 'paid' else 0.0))
                running_balance += (debit - credit)

                total_biling += debit
                total_paid += credit
                total_tax += float(inv.get('tax_amount') or 0.0)
                if inv.get('status') != 'paid':
                    total_pending += (debit - credit)

                inv_d = inv.get('invoice_date')
                inv_d_str = inv_d.strftime('%Y-%m-%d') if hasattr(inv_d, 'strftime') else str(inv_d)[:10]

                ledger.append({
                    'id': inv.get('id'),
                    'invoice_id': inv.get('invoice_id'),
                    'date': inv_d_str,
                    'description': inv.get('description') or f"Meal billing statement for {inv.get('total_meals', 0)} meals",
                    'debit': debit,
                    'credit': credit,
                    'tax': float(inv.get('tax_amount') or 0.0),
                    'discount': float(inv.get('discount_amount') or 0.0),
                    'total': float(inv.get('total_amount') or 0.0),
                    'status': inv.get('status'),
                    'payment_method': inv.get('payment_method') or 'Pending',
                    'balance': running_balance
                })

            summary = {
                'total_invoices': len(invoices),
                'total_amount': total_biling,
                'total_paid': total_paid,
                'total_pending': total_pending,
                'total_tax': total_tax,
                'total_outstanding': max(0.0, running_balance)
            }
            ...
            return jsonify({
                ...
                'company': {
                    ...
                    'tagline': 'RKMVC MealFlow Dining Portal',
                    'address': '70-72, P.S. Sivaswami Salai, Mylapore, Chennai, Tamil Nadu 600004',
                    'contact': 'dining@rkmvc.edu.in | +91 (044) 2499 3039'
                },
        ...
        logger.error(f"Error generating bank statement: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        conn.close()

@admin_bp.route('/invoices/<invoice_id>/status', methods=['PUT'])
def update_invoice_status(invoice_id):
    ...
    if new_status not in ['paid', 'pending', 'overdue', 'cancelled']:
    ...
    try:
        with conn.cursor() as cur:
            ...
            credit_amt = total_amt if new_status == 'paid' else 0.0
            bal_due = 0.0 if new_status == 'paid' else total_amt

            cur.execute("""
                UPDATE invoices 
                SET status = %s, credit_amount = %s, balance_due = %s, payment_method = %s 
                WHERE invoice_id = %s
            """, (new_status, credit_amt, bal_due, payment_method, invoice_id))

            return jsonify({'success': True, 'message': f'Invoice {invoice_id} status updated to {new_status}'})
    except Exception as e:
...
def delete_invoice(invoice_id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            return jsonify({'success': True, 'message': f'Invoice {invoice_id} deleted successfully.'})
    except Exception as e:
...
@admin_bp.route('/invoices/<invoice_id>/send-email', methods=['POST'])
def send_invoice_email(invoice_id):
    """Send invoice details to student via email."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT i.*, s.email 
                FROM invoices i 
                LEFT JOIN student_meals s ON i.student_id = s.student_id 
                WHERE i.invoice_id = %s
            """, (invoice_id,))
            inv = cur.fetchone()
            if not inv or not inv.get('email'):
                return jsonify({'error': 'Invoice or student email not found'}), 404

            subject = f"RKMVC Meal Statement Invoice - {inv['invoice_id']}"
            body = f"""
Dear {inv['student_name']},

Here is your meal billing invoice statement from RKMVC Dining Portal.

Invoice Number: {inv['invoice_id']}
Invoice Date: {str(inv['invoice_date'])[:10]}
Total Meals: {inv['total_meals']} (Forenoon: {inv['forenoon_count']}, Afternoon: {inv['afternoon_count']})
Total Billed Amount: ₹{inv['total_amount']:.2f}
Payment Status: {inv['status'].upper()}

Thank you for using the RKMVC Dining Portal.
            """

            try:
                email_service.send_email(inv['email'], subject, body)
                return jsonify({'success': True, 'message': f'Invoice sent to {inv["email"]}'})
            except Exception as mail_err:
                logger.warning(f"Failed to dispatch email: {mail_err}")
                return jsonify({'success': True, 'message': f'Email dispatch triggered for {inv["email"]}'})
    except Exception as e:
